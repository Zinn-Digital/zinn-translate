...
define( 'ZINN_TRANSLATE_VERSION', '2.2.10' );
...
// ⛔⛔ THESE THREE LOAD BEFORE `admin-ui`, AND WITHOUT THEM THE SETTINGS SCREEN IS A PHP
// FATAL. `class-zinn-translate-admin-ui.php` is GENERATED, and it calls
// `Zinn_Translate_Admin_Fields::`, `Zinn_Translate_Connection::` and
// `Zinn_Translate_Diagnostics::` statically — on classes that shipped in 2.1.1 and were
// require_once'd by nothing. Opening (or saving) the plugin's settings page called a static
// method on an undefined class: fatal, on every install that had auto-updated.
//
// ⛔ Nothing could see it. `build-admin-ui.php --check` proves every generated COPY is
// current, never that anything LOADS what the copy calls; `test-wp-compat.sh` never opens a
// settings page; and the plugin activates fine, because the fatal is on a screen nobody
// visits during activation.
//
// ⭐ Ordering is load-bearing: a `require_once` AFTER the file that uses it still works at
// call time, but keeping them above it is what makes the dependency readable — the generated
// file's needs are satisfied before it arrives.
require_once __DIR__ . '/includes/class-zinn-translate-admin-fields.php';
require_once __DIR__ . '/includes/class-zinn-translate-connection.php';
require_once __DIR__ . '/includes/class-zinn-translate-diagnostics.php';
